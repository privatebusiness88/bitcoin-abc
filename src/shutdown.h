...
/**
 * Initialize shutdown state. This must be called before using either
...
/**
 * Clear shutdown flag. Only use this during init (before calling
 * WaitForShutdown in any thread), or in the unit tests. Calling it in other
 * circumstances will cause a race condition.
 */
void AbortShutdown();
