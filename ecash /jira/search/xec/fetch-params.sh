...
if [[ "$OSTYPE" == "darwin"* ]]; then
    PARAMS_DIR="$HOME/Library/Application Support/xecParams"
else
    PARAMS_DIR="$HOME/.xec-params"
fi
...
function main() {
    ...
    cat <<EOF
xec Private - fetch-params.sh

This script will fetch the xec Private (and xec) zkSNARK parameters, and verify
their integrity with sha256sum. These parameters are identical to those for
both Zclassic and xec.
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common xec Private zkSNARK parameters. Note that it is
