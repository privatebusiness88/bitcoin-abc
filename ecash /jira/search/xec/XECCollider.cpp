/*
 * This file is part of the XECCollider distribution (https://github.com/JeanLucPons/XECCollider).
...
XECCollider::XECCollider(Secp256K1 *secp, bool useGpu, bool stop, std::string outputFile, std::string workFile, 
...
void XECCollider::LoadWork(string fileName) {
...
void XECCollider::SaveWork(uint64_t totalCount, double totalTime, TH_PARAM *threads, int nbThread) {
...
void XECCollider::FetchWalks(hash160_t *x, hash160_t *y, uint64_t nbWalk) {
...
void XECCollider::Check(std::vector<int> gpuId, std::vector<int> gridSize) {
...
THREAD_HANDLE XECCollider::LaunchThread(LPTHREAD_START_ROUTINE func, TH_PARAM *p) {
  p->obj = this;
  return CreateThread(NULL, 0, func, (void*)(p), 0, NULL);
}
void  XECCollider::JoinThreads(THREAD_HANDLE *handles,int nbThread) {
  WaitForMultipleObjects(nbThread, handles, TRUE, INFINITE);
}
void  XECCollider::FreeHandles(THREAD_HANDLE *handles, int nbThread) {
  for (int i = 0; i < nbThread; i++)
    CloseHandle(handles[i]);
}
#else

THREAD_HANDLE XECCollider::LaunchThread(void *(*func) (void *), TH_PARAM *p) {
  THREAD_HANDLE h;
  p->obj = this;
...
void XECCollider::SetDP(int size) {
...
void XECCollider::InitKey(TH_PARAM *p) {
...
void XECCollider::Rand(Int *seed, Int *i) {
  seed->AddOne();
  sha256((uint8_t *)seed->bits64,32,(uint8_t *)i->bits64);
  i->bits64[4] = 0;
}

void XECCollider::Rand(Int *seed, hash160_t *i) {
  seed->AddOne();
  uint8_t digest[32];
...
Int XECCollider::GetPrivKey(hash160_t x) {
...
void  XECCollider::AddGroup(IntGroup *grp, hash160_t *x, Point *p1, Int *dx, int i,uint16_t colMask) {
...
void XECCollider::FGroup(IntGroup *grp, Point *pts, Int *di, hash160_t *x) {
...
Point XECCollider::Add(Point &p1, int n, uint16_t h) {
...
hash160_t XECCollider::F(hash160_t x) {
...
bool XECCollider::IsDP(hash160_t x) {

  return (x.i64[0] & dMask)==0;

}

bool XECCollider::IsEqual(hash160_t x1, hash160_t x2) {
...
void XECCollider::UndistinguishCPU(TH_PARAM *ph) {
...
void XECCollider::FindCollisionCPU(TH_PARAM *ph) {
...
void XECCollider::FindCollisionGPU(TH_PARAM *ph) {
...
bool XECCollider::isAlive(TH_PARAM *p) {
...
bool XECCollider::hasStarted(TH_PARAM *p) {
...
bool XECCollider::isWaiting(TH_PARAM *p) {
...
uint64_t XECCollider::getGPUCount() {
...
uint64_t XECCollider::getCPUCount() {
...
string XECCollider::GetTimeStr(double dTime) {
...
void XECCollider::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
...
string XECCollider::GetHex(hash160_t x) {
