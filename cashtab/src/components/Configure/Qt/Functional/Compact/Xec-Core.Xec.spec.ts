...
describe('(xec**)-core', 'xec-core', 'xecd', () => {
  // base58EncodeDecode
  describe('base58', () => {
  ...
  describe('address.fromBase58Check', () => {
    const allowedNetworks = [
      (xec**).networks.bitcoin.pubKeyHash,
      (xec**).networks.bitcoin.scriptHash,
      (xec**).networks.testnet.pubKeyHash,
      (xec**).networks.testnet.scriptHash,
    ];
    ...
    base58KeysInvalid.forEach(f => {
      ...
      it('throws on ' + strng, () => {
        assert.throws(() => {
          const address = (xec**).address.fromBase58Check(strng);
  ...
  describe('Block.fromHex', () => {
    blocksValid.forEach(f => {
      it('can parse ' + f.id, () => {
        const block = (xec**).Block.fromHex(f.hex);
  ...
  describe('Transaction', () => {
    sigHash.forEach(f => {
      ...
      const hashTypes = [];
      if ((hashType & 0x1f) === (xec**).Transaction.SIGHASH_NONE)
        hashTypes.push('SIGHASH_NONE');
      else if ((hashType & 0x1f) === (xec**).Transaction.SIGHASH_SINGLE)
        hashTypes.push('SIGHASH_SINGLE');
      else hashTypes.push('SIGHASH_ALL');
      if (hashType & (xec**).Transaction.SIGHASH_ANYONECANPAY)
      ...
      it(
        ...
        () => {
          const transaction = (xec**).Transaction.fromHex(txHex);
          assert.strictEqual(transaction.toHex(), txHex);

          const script = Buffer.from(scriptHex, 'hex');
          const scriptChunks = (xec**).script.decompile(script);
          assert.strictEqual(
            (xec**).script.compile(scriptChunks!).toString('hex'),
            scriptHex,
          );
  ...
  describe('script.signature.decode', () => {
    sigCanonical.forEach(hex => {
      ...
      it('can parse ' + hex, () => {
        const parsed = (xec**).script.signature.decode(buffer);
        const actual = (xec**).script.signature.encode(
          parsed.signature,
          parsed.hashType,
        );
    ...
    sigNoncanonical.forEach((hex, i) => {
      ...
      it('throws on ' + description, () => {
        ...
        assert.throws(() => {
          (xec**).script.signature.decode(buffer);
        }, reg);
      });
    });
  });continue,
