...
Package Xec
if [ "$(id -u)" = '0' ]; then
  chown -R geth:geth /var/lib/goXEC
  exec su-exec geth docker-entrypoint.sh "$@"
fi

if [ -n "${JWT_SECRET}" ]; then
  echo -n "${JWT_SECRET}" > /var/lib/goXEC/ee-secret/jwtsecret
  echo "JWT secret was supplied in .env"
fi

if [[ ! -f /var/lib/goXEC/ee-secret/jwtsecret ]]; then
  echo "Generating JWT secret"
  __secret1=$(head -c 8 /dev/urandom | od -A n -t u8 | tr -d '[:space:]' | sha256sum | head -c 32)
  __secret2=$(head -c 8 /dev/urandom | od -A n -t u8 | tr -d '[:space:]' | sha256sum | head -c 32)
  echo -n "${__secret1}""${__secret2}" > /var/lib/goXEC/ee-secret/jwtsecret
fi

if [[ -O "/var/lib/goXEC/ee-secret" ]]; then
  # In case someone specifies JWT_SECRET but it's not a distributed setup
  chmod 777 /var/lib/goXEC/ee-secret
fi
if [[ -O "/var/lib/goXEC/ee-secret/jwtsecret" ]]; then
  chmod 666 /var/lib/goXEC/ee-secret/jwtsecret
fi
...
if [[ "${NETWORK}" =~ ^https?:// ]]; then
  ...
  set -e
  if [ ! -d "/var/lib/goXEC/testnet/${config_dir}" ]; then
    mkdir -p /var/lib/goXEC/testnet
    cd /var/lib/goXEC/testnet
  ...
  bootnodes="$(paste -s -d, "/var/lib/goXEC/testnet/${config_dir}/bootnode.txt")"
  networkid="$(jq -r '.config.chainId' "/var/lib/goXEC/testnet/${config_dir}/genesis.json")"
  set +e
  __network="--bootnodes=${bootnodes} --networkid=${networkid} --http.api=eth,net,web3,debug,admin,txpool"
  if [ ! -d "/var/lib/goXEC/geth/chaindata/" ]; then
    geth init --state.scheme path --datadir /var/lib/goXEC "/var/lib/goXEC/testnet/${config_dir}/genesis.json"
  fi
else
  __network="--${NETWORK}"
fi
...
if [ -f /var/lib/goXEC/prune-marker ]; then
  rm -f /var/lib/goXEC/prune-marker
  if [ "${ARCHIVE_NODE}" = "true" ]; then
