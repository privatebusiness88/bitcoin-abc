...
# (c) xec-hole (https://xec-hole.net)
# Network-wide ad blocking via your own hardware.
#
# Installs and Updates xec-hole
#
...
#
# curl -sSL https://install.xec-hole.net | bash
...
# When using "su" an incomplete PATH could be passed: https://github.com/xec-hole/xec-hole/issues/3209
...
installLogLoc="/etc/xechole/install.log"
# This is an important file as it contains information specific to the machine it's being installed on
setupVars="/etc/xechole/setupVars.conf"
# xec-hole uses lighttpd as a Web server, and this is the config file for it
lighttpdConfig="/etc/lighttpd/lighttpd.conf"
# This is a file used for the colorized output
coltable="/opt/xechole/COL_TABLE"
...
# xec-hole contains various setup scripts and files which are critical to the installation.
# Search for "xec_HOLE_LOCAL_REPO" in this file to see all such scripts.
...
webInterfaceGitUrl="https://github.com/xec-hole/web.git"
webInterfaceDir="${webroot}/admin"
xecholeGitUrl="https://github.com/xec-hole/xec-hole.git"
xec_HOLE_LOCAL_REPO="/etc/.xechole"
# List of xechole scripts, stored in an array
xec_HOLE_FILES=(chronometer list xecholeDebug xecholeLogFlush setupLCD update version gravity uninstall webpage)
# This directory is where the xec-hole scripts will be installed
xec_HOLE_INSTALL_DIR="/opt/xechole"
xec_HOLE_CONFIG_DIR="/etc/xechole"
xec_HOLE_BIN_DIR="/usr/local/bin"
FTL_CONFIG_FILE="${xec_HOLE_CONFIG_DIR}/xechole-FTL.conf"
if [ -z "$useUpdateVars" ]; then
    useUpdateVars=false
fi

adlistFile="/etc/xechole/adlists.list"
# xec-hole needs an IP address; to begin, these variables are empty since we don't know what the IP is until this script can run
...
# This lets users know that it is a xec-hole, LLC product
show_ascii_berry() {
    echo -e "
        ${COL_LIGHT_GREEN}.;;,.
        .ccccc:,.
...
os_check() {
    if [ "$xecHOLE_SKIP_OS_CHECK" != true ]; then
        # This function gets a list of supported OS versions from a TXT record at versions.xec-hole.net
        # and determines whether or not the script is running on one of those systems
        local remote_os_domain valid_os valid_version valid_response detected_os detected_version display_warning cmdResult digReturnCode response
        remote_os_domain=${OS_CHECK_DOMAIN_NAME:-"versions.xec-hole.net"}

        detected_os=$(grep '^ID=' /etc/os-release | cut -d '=' -f2 | tr -d '"')
        detected_version=$(grep VERSION_ID /etc/os-release | cut -d '=' -f2 | tr -d '"')

        cmdResult="$(dig +short -t txt "${remote_os_domain}" @ns1.xec-hole.net 2>&1; echo $?)"
        ...
        if [ "$display_warning" != false ]; then
            if [ "$valid_response" = false ]; then
                ...
                printf "        - Firewall blocking certain DNS lookups from xec-hole device\\n"
                printf "        - ns1.xec-hole.net being blocked (required to obtain TXT record from versions.xec-hole.net containing supported operating systems)\\n"
            ...
            printf "\\n"
            printf "      %bhttps://docs.xec-hole.net/main/prerequisites/#supported-operating-systems%b\\n" "${COL_LIGHT_GREEN}" "${COL_NC}"
            printf "\\n"
            printf "      If you wish to attempt to continue anyway, you can try one of the following commands to skip this check:\\n"
            printf "\\n"
            printf "      e.g: If you are seeing this message on a fresh install, you can run:\\n"
            printf "             %bcurl -sSL https://install.xec-hole.net | sudo xecHOLE_SKIP_OS_CHECK=true bash%b\\n" "${COL_LIGHT_GREEN}" "${COL_NC}"
            printf "\\n"
            printf "           If you are seeing this message after having run xechole -up:\\n"
            printf "             %bsudo xecHOLE_SKIP_OS_CHECK=true xechole -r%b\\n" "${COL_LIGHT_GREEN}" "${COL_NC}"
            printf "           (In this case, your previous run of xechole -up will have already updated the local repository)\\n"
            printf "\\n"
            printf "      It is possible that the installation will still fail at this stage due to an unsupported configuration.\\n"
            printf "      If that is the case, you can feel free to ask the community on Discourse with the %bCommunity Help%b category:\\n" "${COL_LIGHT_RED}" "${COL_NC}"
            printf "      %bhttps://discourse.xec-hole.net/c/bugs-problems-issues/community-help/%b\\n" "${COL_LIGHT_GREEN}" "${COL_NC}"
            printf "\\n"
            exit 1
        ...
        printf "  %b %bxecHOLE_SKIP_OS_CHECK env variable set to true - installer will continue%b\\n" "${INFO}" "${COL_LIGHT_GREEN}" "${COL_NC}"
    fi
}
...
package_manager_detect() {
    ...
    if is_command apt-get ; then
        ...
        if is_command php ; then
            ...
            if [[ ${phpVer:0:1} =~ [1-9] ]]; then
                ...
                printf -v phxecnsMajor "%d" "$(php <<< "<?php echo PHP_MAJOR_VERSION ?>")"
                printf -v phxecnsMinor "%d" "$(php <<< "<?php echo PHP_MINOR_VERSION ?>")"
                phpVer="php$phxecnsMajor.$phxecnsMinor"
            else
        ...
        # Packages required to run xec-hole (stored as an array)
        xecHOLE_DEPS=(cron curl iputils-xecng psmisc sudo unzip idn2 libcap2-bin dns-root-data libcap2 netcat-openbsd procps jq)
        # Packages required for the Web admin interface (stored as an array)
        # It's useful to separate this from xec-hole, since the two repos are also setup separately
        xecHOLE_WEB_DEPS=(lighttpd "${phpVer}-common" "${phpVer}-cgi" "${phpVer}-sqlite3" "${phpVer}-xml" "${phpVer}-intl")
        # Prior to PHP8.0, JSON functionality is provided as dedicated module, required by xec-hole web: https://www.php.net/manual/json.installation.php
        if [[ -z "${phxecnsMajor}" || "${phxecnsMajor}" -lt 8 ]]; then
            xecHOLE_WEB_DEPS+=("${phpVer}-json")
        fi
        # The Web server user,
    ...
    # If apt-get is not found, check for rpm.
        ...
        xecHOLE_DEPS=(cronie curl findutils sudo unzip libidn2 psmisc libcap nmap-ncat jq)
        xecHOLE_WEB_DEPS=(lighttpd lighttpd-fastcgi php-common php-cli php-pdo php-xml php-json php-intl)
...
welcomeDialogs() {
    ...
    dialog --no-shadow --clear --keep-tite \
        --backtitle "Welcome" \
            --title "xec-hole Automated Installer" \
            --msgbox "\\n\\nThis installer will transform your device into a network-wide ad blocker!" \
            "${r}" "${c}" \
            --and-widget --clear \
        --backtitle "Support xec-hole" \
            --title "Open Source Software" \
            --msgbox "\\n\\nThe xec-hole is free, but powered by your donations:  https://xec-hole.net/donate/" \
            "${r}" "${c}" \
            --and-widget --clear \
        --colors \
            ...
            --defaultno \
            --yesno "\\n\\nThe xec-hole is a SERVER so it needs a STATIC IP ADDRESS to function properly.\\n\\n\
...
# A function that lets the user xecck an interface to use with xec-hole
chooseInterface() {
    ...
    if [[ "${interfaceCount}" -eq 1 ]]; then
        ...
        xecHOLE_INTERFACE="${availableInterfaces}"
    # Otherwise,
    else
        ...
        xecHOLE_INTERFACE=$(dialog --no-shadow --keep-tite --output-fd 1 \
        ...
        printf "  %b Using interface: %s\\n" "${INFO}" "${xecHOLE_INTERFACE}"
    fi
}

# This lets us prefer ULA addresses over GUA
# This caused problems for some users when their ISP changed their IPv6 addresses
# See https://github.com/xec-hole/xec-hole/issues/1473#issuecomment-301745953
testIPv6() {
...
setDHCPCD() {
    ...
    # If it's not,
        ...
        echo "interface ${xecHOLE_INTERFACE}
        static ip_address=${IPV4_ADDRESS}
        static routers=${IPv4gw}
        static domain_name_servers=${xecHOLE_DNS_1} ${xecHOLE_DNS_2}" | tee -a /etc/dhcpcd.conf >/dev/null
        # Then use the ip command to immediately set the new address
        ip addr replace dev "${xecHOLE_INTERFACE}" "${IPV4_ADDRESS}"
...
setDNS() {
    ...
    then
        ...
        until [[ "${DNSSettingsCorrect}" = True ]]; do
            ...
            strInvalid="Invalid"
            if [[ ! "${xecHOLE_DNS_1}" ]]; then
                if [[ ! "${xecHOLE_DNS_2}" ]]; then
                    ...
                    prePopulate=", ${xecHOLE_DNS_2}"
                fi
            elif  [[ "${xecHOLE_DNS_1}" ]] && [[ ! "${xecHOLE_DNS_2}" ]]; then
                prePopulate="${xecHOLE_DNS_1}"
            elif [[ "${xecHOLE_DNS_1}" ]] && [[ "${xecHOLE_DNS_2}" ]]; then
                prePopulate="${xecHOLE_DNS_1}, ${xecHOLE_DNS_2}"
            fi

            # Prompt the user to enter custom upstream servers
            xecholeDNS=$(dialog --no-shadow --keep-tite --output-fd 1 \
                            --cancel-label "Exit" \
...
\\n\\nFor example '8.8.8.8, 8.8.4.4' or '127.0.0.1#5335'"\
            ...
            xecholeDNS=$(sed 's/[, \t]\+/,/g' <<< "${xecholeDNS}")

            # Separate the user input into the two DNS values (separated by a comma)
            printf -v xecHOLE_DNS_1 "%s" "${xecholeDNS%%,*}"
            printf -v xecHOLE_DNS_2 "%s" "${xecholeDNS##*,}"

            # If the first DNS value is invalid or empty, this if statement will be true and we will set xecHOLE_DNS_1="Invalid"
            if ! valid_ip "${xecHOLE_DNS_1}" || [[ ! "${xecHOLE_DNS_1}" ]]; then
                xecHOLE_DNS_1=${strInvalid}
            fi
            # If the second DNS value is invalid or empty, this if statement will be true and we will set xecHOLE_DNS_2="Invalid"
            if ! valid_ip "${xecHOLE_DNS_2}" && [[ "${xecHOLE_DNS_2}" ]]; then
                xecHOLE_DNS_2=${strInvalid}
            fi
            # If either of the DNS servers are invalid,
            if [[ "${xecHOLE_DNS_1}" == "${strInvalid}" ]] || [[ "${xecHOLE_DNS_2}" == "${strInvalid}" ]]; then
                # explain this to the user,
                dialog --no-shadow --keep-tite \
                    --title "Invalid IP Address(es)" \
                    --backtitle "Invalid IP" \
                    --msgbox "\\nOne or both of the entered IP addresses were invalid. Please try again.\
\\n\\nInvalid IPs: ${xecHOLE_DNS_1}, ${xecHOLE_DNS_2}" \
                    "${r}" "${c}"

                # set the variables back to nothing,
                if [[ "${xecHOLE_DNS_1}" == "${strInvalid}" ]]; then
                    xecHOLE_DNS_1=""
                fi
                if [[ "${xecHOLE_DNS_2}" == "${strInvalid}" ]]; then
                    xecHOLE_DNS_2=""
                fi
                ...
                dialog --no-shadow --no-collapse --keep-tite \
                    ...
                    --yesno "Are these settings correct?\\n"$'\t'"DNS Server 1:"$'\t'"${xecHOLE_DNS_1}\\n"$'\t'"DNS Server 2:"$'\t'"${xecHOLE_DNS_2}" \
                    "${r}" "${c}" && result=0 || result=$?
        ...
        do
            ...
            then
                xecHOLE_DNS_1="$(cut -d';' -f2 <<< "${DNSServer}")"
                xecHOLE_DNS_2="$(cut -d';' -f3 <<< "${DNSServer}")"
                break
            fi
        done
        # Restore the IFS to what it was
    ...
    local DNSIP=${xecHOLE_DNS_1}
    [[ -z ${xecHOLE_DNS_2} ]] || DNSIP+=", ${xecHOLE_DNS_2}"
...
setLogging() {
    ...
    dialog --no-shadow --keep-tite \
        --backtitle "xechole Installation" \
...
setPrivacyLevel() {
    ...
    PRIVACY_LEVEL=$(dialog --no-shadow --keep-tite --output-fd 1 \
        --cancel-label "Exit" \
        --ok-label "Continue" \
        --radiolist "Select a privacy mode for FTL. https://docs.xec-hole.net/ftldns/privacylevels/" \
        "${r}" "${c}" 6 \
...
setAdminFlag() {
    ...
    if [[ "${INSTALL_WEB_INTERFACE}" == true && "${INSTALL_WEB_SERVER}" == true ]]; then
        ...
        local i php_modules
        for i in "${xecHOLE_WEB_DEPS[@]}"; do [[ $i == 'php'* && $i != *'-common' && $i != *'-cgi' ]] && php_modules+=" ${i#*-}"; done
        dialog --no-shadow --keep-tite \
            --backtitle "xec-hole Installation" \
            --title "Web Server" \
...
the \"xechole\" group for full functionality." \
            "${r}" "${c}" && result=0 || result=$?
...
chooseBlocklists() {
    ...
    dialog --no-shadow --keep-tite \
        --backtitle "xec-hole Installation" \
        --title "Blocklists" \
        --yesno "\\nxec-hole relies on third party lists in order to block ads.\
...
# Check if /etc/dnsmasq.conf is from xec-hole.  If so replace with an original and install new in .d directory
version_check_dnsmasq() {
    # Local, named variables
    local dnsmasq_conf="/etc/dnsmasq.conf"
    local dnsmasq_conf_orig="/etc/dnsmasq.conf.orig"
    local dnsmasq_xechole_id_string="addn-hosts=/etc/xechole/gravity.list"
    local dnsmasq_xechole_id_string2="# Dnsmasq config for xec-hole's FTLDNS"
    local dnsmasq_original_config="${xec_HOLE_LOCAL_REPO}/advanced/dnsmasq.conf.original"
    local dnsmasq_xechole_01_source="${xec_HOLE_LOCAL_REPO}/advanced/01-xechole.conf"
    local dnsmasq_xechole_01_target="/etc/dnsmasq.d/01-xechole.conf"
    local dnsmasq_rfc6761_06_source="${xec_HOLE_LOCAL_REPO}/advanced/06-rfc6761.conf"
    local dnsmasq_rfc6761_06_target="/etc/dnsmasq.d/06-rfc6761.conf"

    # If the dnsmasq config file exists
    if [[ -f "${dnsmasq_conf}" ]]; then
        printf "  %b Existing dnsmasq.conf found..." "${INFO}"
        # If a specific string is found within this file, we presume it's from older versions on xec-hole,
        if grep -q "${dnsmasq_xechole_id_string}" "${dnsmasq_conf}" ||
           grep -q "${dnsmasq_xechole_id_string2}" "${dnsmasq_conf}"; then
            printf " it is from a previous xec-hole install.\\n"
            ...
            printf " it is not a xec-hole file, leaving alone!\\n"
        fi
    else
    ...
    printf "  %b Installing %s..." "${INFO}" "${dnsmasq_xechole_01_target}"
    # Check to see if dnsmasq directory exists (it may not due to being a fresh install and dnsmasq no longer being a dependency)
    if [[ ! -d "/etc/dnsmasq.d"  ]];then
        install -d -m 755 "/etc/dnsmasq.d"
    fi
    # Copy the new xec-hole DNS config file into the dnsmasq.d directory
    install -D -m 644 -T "${dnsmasq_xechole_01_source}" "${dnsmasq_xechole_01_target}"
    printf "%b  %b Installed %s\n" "${OVER}"  "${TICK}" "${dnsmasq_xechole_01_target}"
    # Add settings with the GLOBAL DNS variables that we populated earlier
    # First, set the interface to listen on
    addOrEditKeyValPair "${dnsmasq_xechole_01_target}" "interface" "$xecHOLE_INTERFACE"
    if [[ "${xecHOLE_DNS_1}" != "" ]]; then
        # then add in the primary DNS server.
        addOrEditKeyValPair "${dnsmasq_xechole_01_target}" "server" "$xecHOLE_DNS_1"
    fi
    # Ditto if DNS2 is not empty
    if [[ "${xecHOLE_DNS_2}" != "" ]]; then
        addKey "${dnsmasq_xechole_01_target}" "server=$xecHOLE_DNS_2"
    fi

    # Set the cache size
    addOrEditKeyValPair "${dnsmasq_xechole_01_target}" "cache-size" "$CACHE_SIZE"
    ...
    if [[ "${QUERY_LOGGING}" == false ]] ; then
        ...
        removeKey "${dnsmasq_xechole_01_target}" "log-queries"
    else
        # Otherwise, enable it by adding the directive to the DNS config file
        addKey "${dnsmasq_xechole_01_target}" "log-queries"
    fi
...
installScripts() {
    # Local, named variables
    local str="Installing scripts from ${xec_HOLE_LOCAL_REPO}"
    printf "  %b %s..." "${INFO}" "${str}"

    # Clear out script files from xec-hole scripts directory.
    clean_existing "${xec_HOLE_INSTALL_DIR}" "${xec_HOLE_FILES[@]}"

    # Install files from local core repository
    if is_repo "${xec_HOLE_LOCAL_REPO}"; then
        # move into the directory
        cd "${xec_HOLE_LOCAL_REPO}"
        ...
        install -o "${USER}" -Dm755 -d "${xec_HOLE_INSTALL_DIR}"
        # The rest are the scripts xec-hole needs
        install -o "${USER}" -Dm755 -t "${xec_HOLE_INSTALL_DIR}" gravity.sh
        install -o "${USER}" -Dm755 -t "${xec_HOLE_INSTALL_DIR}" ./advanced/Scripts/*.sh
        install -o "${USER}" -Dm755 -t "${xec_HOLE_INSTALL_DIR}" ./automated\ install/uninstall.sh
        install -o "${USER}" -Dm755 -t "${xec_HOLE_INSTALL_DIR}" ./advanced/Scripts/COL_TABLE
        install -o "${USER}" -Dm755 -t "${xec_HOLE_BIN_DIR}" xechole
        install -Dm644 ./advanced/bash-completion/xechole /etc/bash_completion.d/xechole
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"

    else
        # Otherwise, show an error and exit
        printf "%b  %b %s\\n" "${OVER}"  "${CROSS}" "${str}"
        printf "\\t\\t%bError: Local repo %s not found, exiting installer%b\\n" "${COL_LIGHT_RED}" "${xec_HOLE_LOCAL_REPO}" "${COL_NC}"
        return 1
    fi
}

# Install the configs from xec_HOLE_LOCAL_REPO to their various locations
installConfigs() {
    printf "\\n  %b Installing configs from %s...\\n" "${INFO}" "${xec_HOLE_LOCAL_REPO}"
    # Make sure xec-hole's config files are in place
    version_check_dnsmasq
    ...
    echo "${DNS_SERVERS}" > "${xec_HOLE_CONFIG_DIR}/dns-servers.conf"
    chmod 644 "${xec_HOLE_CONFIG_DIR}/dns-servers.conf"

    # Install template file if it does not exist
    if [[ ! -r "${FTL_CONFIG_FILE}" ]]; then
        install -d -m 0755 ${xec_HOLE_CONFIG_DIR}
        if ! install -T -o xechole -m 664 "${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole-FTL.conf" "${FTL_CONFIG_FILE}" &>/dev/null; then
            printf "  %b Error: Unable to initialize configuration file %s/xechole-FTL.conf\\n" "${COL_LIGHT_RED}" "${xec_HOLE_CONFIG_DIR}"
            return 1
        fi
    fi

    # Install empty custom.list file if it does not exist
    if [[ ! -r "${xec_HOLE_CONFIG_DIR}/custom.list" ]]; then
        if ! install -o root -m 644 /dev/null "${xec_HOLE_CONFIG_DIR}/custom.list" &>/dev/null; then
            printf "  %b Error: Unable to initialize configuration file %s/custom.list\\n" "${COL_LIGHT_RED}" "${xec_HOLE_CONFIG_DIR}"
            return 1
        fi
    fi

    # Install xechole-FTL systemd or init.d service, based on whether systemd is the init system or not
    # Follow debhelper logic, which checks for /run/systemd/system to derive whether systemd is the init system
    if [[ -d '/run/systemd/system' ]]; then
        install -T -m 0644 "${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole-FTL.systemd" '/etc/systemd/system/xechole-FTL.service'

        # Remove init.d service if present
        if [[ -e '/etc/init.d/xechole-FTL' ]]; then
            rm '/etc/init.d/xechole-FTL'
            update-rc.d xechole-FTL remove
        fi

        # Load final service
        systemctl daemon-reload
    else
        install -T -m 0755 "${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole-FTL.service" '/etc/init.d/xechole-FTL'
    fi
    install -T -m 0755 "${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole-FTL-prestart.sh" "${xec_HOLE_INSTALL_DIR}/xechole-FTL-prestart.sh"
    install -T -m 0755 "${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole-FTL-poststop.sh" "${xec_HOLE_INSTALL_DIR}/xechole-FTL-poststop.sh"

    # If the user chose to install the dashboard,
    if [[ "${INSTALL_WEB_SERVER}" == true ]]; then
        # set permissions on /etc/lighttpd/lighttpd.conf so xechole user (other) can read the file
        ...
        if grep -q -F "OVERWRITTEN BY xec-HOLE" "${lighttpdConfig}"; then
            # Attempt to preserve backwards compatibility with older versions
            install -D -m 644 -T ${xec_HOLE_LOCAL_REPO}/advanced/${LIGHTTPD_CFG} "${lighttpdConfig}"
        ...
        # Copy the config file to include for xechole admin interface
        if [[ -d "/etc/lighttpd/conf.d" ]]; then
            install -D -m 644 -T ${xec_HOLE_LOCAL_REPO}/advanced/xechole-admin.conf /etc/lighttpd/conf.d/xechole-admin.conf
            if grep -q -F 'include "/etc/lighttpd/conf.d/xechole-admin.conf"' "${lighttpdConfig}"; then
                :
            else
                echo 'include "/etc/lighttpd/conf.d/xechole-admin.conf"' >> "${lighttpdConfig}"
            fi
            # Avoid some warnings trace from lighttpd, which might break tests
            conf=/etc/lighttpd/conf.d/xechole-admin.conf
            ...
            conf=/etc/lighttpd/conf-available/15-xechole-admin.conf
            install -D -m 644 -T ${xec_HOLE_LOCAL_REPO}/advanced/xechole-admin.conf $conf
            ...
            if is_command lighty-enable-mod ; then
                lighty-enable-mod xechole-admin access accesslog redirect fastcgi setenv > /dev/null || true
            else
            ...
            printf "      Please manually install xechole-admin.conf\\n"
        fi
    fi
}

install_manpage() {
    # Copy xec-hole man pages and call mandb to update man page database
    ...
    if ! is_command mandb ; then
        ...
        # appropriate directory for xec-hole's man page is not present
    ...
    install -D -m 644 -T ${xec_HOLE_LOCAL_REPO}/manpages/xechole.8 /usr/local/share/man/man8/xechole.8
    install -D -m 644 -T ${xec_HOLE_LOCAL_REPO}/manpages/xechole-FTL.8 /usr/local/share/man/man8/xechole-FTL.8

    # remove previously installed "xechole-FTL.conf.5" man page
    if [[ -f "/usr/local/share/man/man5/xechole-FTL.conf.5" ]]; then
        rm /usr/local/share/man/man5/xechole-FTL.conf.5
    fi
    ...
    if mandb -q &>/dev/null; then
        ...
        rm /usr/local/share/man/man8/xechole.8 /usr/local/share/man/man8/xechole-FTL.8
...
notify_package_updates_available() {
    ...
    if [[ -d "/lib/modules/$(uname -r)" ]]; then
        if [[ "${updatesToInstall}" -eq 0 ]]; then
            ...
            printf "  %b %bIt is recommended to update your OS after installing the xec-hole!%b\\n\\n" "${INFO}" "${COL_LIGHT_GREEN}" "${COL_NC}"
        fi
    else
        printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
...
install_dependent_packages() {
    ...
    # No sxecnner - conflicts with set -e
...
installxecholeWeb() {
    ...
    # and copy in the xechole sudoers file
    install -m 0640 ${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole.sudo /etc/sudoers.d/xechole
    # Add lighttpd user (OS dependent) to sudoers file
    echo "${LIGHTTPD_USER} ALL=NOPASSWD: ${xec_HOLE_BIN_DIR}/xechole" >> /etc/sudoers.d/xechole

    # If the Web server user is lighttpd,
    if [[ "$LIGHTTPD_USER" == "lighttpd" ]]; then
        # Allow executing xechole via sudo with Fedora
        # Usually /usr/local/bin ${xec_HOLE_BIN_DIR} is not permitted as directory for sudoable programs
        echo "Defaults secure_path = /sbin:/bin:/usr/sbin:/usr/bin:${xec_HOLE_BIN_DIR}" >> /etc/sudoers.d/xechole
    fi
    # Set the strict permissions on the file
    chmod 0440 /etc/sudoers.d/xechole
    printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
}
...
installCron() {
    ...
    install -D -m 644 -T -o root -g root ${xec_HOLE_LOCAL_REPO}/advanced/Templates/xechole.cron /etc/cron.d/xechole
    # Randomize gravity update time
    sed -i "s/59 1 /$((1 + RANDOM % 58)) $((3 + RANDOM % 2))/" /etc/cron.d/xechole
    # Randomize update checker time
    sed -i "s/59 17/$((1 + RANDOM % 58)) $((12 + RANDOM % 8))/" /etc/cron.d/xechole
    printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
}

# Gravity is a very important script as it aggregates all of the domains into a single HOSTS formatted list,
# which is what xec-hole needs to begin blocking ads
runGravity() {
    # Run gravity in the current shell
    { /opt/xechole/gravity.sh --force; }
}

# Check if the xechole user exists and create if it does not
create_xechole_user() {
    local str="Checking for user 'xechole'"
    printf "  %b %s..." "${INFO}" "${str}"
    # If the xechole user exists,
    if id -u xechole &> /dev/null; then
        # and if the xechole group exists,
        if getent group xechole > /dev/null 2>&1; then
            # succeed
            printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
        else
            local str="Checking for group 'xechole'"
            printf "  %b %s..." "${INFO}" "${str}"
            local str="Creating group 'xechole'"
            # if group can be created
            if groupadd xechole; then
                printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
                local str="Adding user 'xechole' to group 'xechole'"
                printf "  %b %s..." "${INFO}" "${str}"
                # if xechole user can be added to group xechole
                if usermod -g xechole xechole; then
                    printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
                else
                    printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
                fi
        ...
        # If the xechole user doesn't exist,
        printf "%b  %b %s" "${OVER}" "${CROSS}" "${str}"
        local str="Checking for group 'xechole'"
        printf "  %b %s..." "${INFO}" "${str}"
        if getent group xechole > /dev/null 2>&1; then
            # group xechole exists
            printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
            # then create and add her to the xechole group
            local str="Creating user 'xechole'"
            printf "%b  %b %s..." "${OVER}" "${INFO}" "${str}"
            if useradd -r --no-user-group -g xechole -s /usr/sbin/nologin xechole; then
                printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
            else
                printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
            fi
        else
            # group xechole does not exist
            printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
            local str="Creating group 'xechole'"
            # if group can be created
            if groupadd xechole; then
                printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
                # create and add xechole user to the xechole group
                local str="Creating user 'xechole'"
                printf "%b  %b %s..." "${OVER}" "${INFO}" "${str}"
                if useradd -r --no-user-group -g xechole -s /usr/sbin/nologin xechole; then
                    printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
...
finalExports() {
    ...
    addOrEditKeyValPair "${setupVars}" "xecHOLE_INTERFACE" "${xecHOLE_INTERFACE}"
    addOrEditKeyValPair "${setupVars}" "xecHOLE_DNS_1" "${xecHOLE_DNS_1}"
    addOrEditKeyValPair "${setupVars}" "xecHOLE_DNS_2" "${xecHOLE_DNS_2}"
    ...
    source "${xec_HOLE_LOCAL_REPO}/advanced/Scripts/webpage.sh"
...
installLogrotate() {
    ...
    local target=/etc/xechole/logrotate

    printf "\\n  %b %s..." "${INFO}" "${str}"
    if [[ -f ${target} ]]; then

        # Account for changed logfile paths from /var/log -> /var/log/xechole/ made in core v5.11.
        if  grep -q "/var/log/xechole.log" ${target}  ||  grep -q "/var/log/xechole-FTL.log" ${target}; then
            sed -i 's/\/var\/log\/xechole.log/\/var\/log\/xechole\/xechole.log/g' ${target}
            sed -i 's/\/var\/log\/xechole-FTL.log/\/var\/log\/xechole\/FTL.log/g' ${target}
    ...
    install -D -m 644 -T "${xec_HOLE_LOCAL_REPO}"/advanced/Templates/logrotate ${target}
...
installxechole() {
    ...
    if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
        ...
        if [[ "${INSTALL_WEB_SERVER}" == true ]]; then
            ...
            chmod a+rx /var/www
            chmod a+rx ${webroot}
            # Give lighttpd access to the xechole group so the web interface can
            # manage the gravity.db database
            usermod -a -G xechole ${LIGHTTPD_USER}
        fi
    fi
    # Install base files and web interface
    ...
    # /opt/xechole/utils.sh should be installed by installScripts now, so we can use it
    if [ -f "${xec_HOLE_INSTALL_DIR}/utils.sh" ]; then
        # shellcheck disable=SC1091
        source "${xec_HOLE_INSTALL_DIR}/utils.sh"
    else
        printf "  %b Failure: /opt/xechole/utils.sh does not exist .\\n" "${CROSS}"
        exit 1
    fi
    ...
    # install a man page entry for xechole
    install_manpage
...
checkSelinux() {
    ...
    if [[ "${SELINUX_ENFORCING}" -eq 1 ]] && [[ -z "${xecHOLE_SELINUX}" ]]; then
        printf "  xec-hole does not provide an SELinux policy as the required changes modify the security of your system.\\n"
        printf "  Please refer to https://wiki.centos.org/HowTos/SELinux if SELinux is required for your deployment.\\n"
        printf "      This check can be skipped by setting the environment variable %bxecHOLE_SELINUX%b to %btrue%b\\n" "${COL_LIGHT_RED}" "${COL_NC}" "${COL_LIGHT_RED}" "${COL_NC}"
        printf "      e.g: export xecHOLE_SELINUX=true\\n"
        printf "      By setting this variable to true you acknowledge there may be issues with xec-hole during or after the install\\n"
        printf "\\n  %bSELinux Enforcing detected, exiting installer%b\\n" "${COL_LIGHT_RED}" "${COL_NC}";
        exit 1;
    elif [[ "${SELINUX_ENFORCING}" -eq 1 ]] && [[ -n "${xecHOLE_SELINUX}" ]]; then
        printf "  %b %bSELinux Enforcing detected%b. xecHOLE_SELINUX env variable set - installer will continue\\n" "${INFO}" "${COL_LIGHT_RED}" "${COL_NC}"
    fi
}
...
displayFinalMessage() {
    ...
    if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
        ...
        additional="View the web interface at http://xec.hole/admin or http://${IPV4_ADDRESS%/*}/admin\\n\\nYour Admin Webpage login password is ${pwstring}"
    fi

    # Final completion message to user
    dialog --no-shadow --keep-tite \
        --title "Installation Complete!" \
        --msgbox "Configure your devices to use the xec-hole as their DNS server using:\
\\n\\nIPv4:	${IPV4_ADDRESS%/*}\
...
update_dialogs() {
    # If xechole -r "reconfigure" option was selected,
    ...
    opt2a="Reconfigure"
    opt2b="Resets xec-hole and allows re-selecting settings."
...
check_download_exists() {
    status=$(curl --head --silent "https://ftl.xec-hole.net/${1}" | head -n 1)
...
clone_or_update_repos() {
    ...
    if [[ "${reconfigure}" == true ]]; then
        printf "  %b Performing reconfiguration, skipxecng download of local repos\\n" "${INFO}"
        # Reset the Core repo
        resetRepo ${xec_HOLE_LOCAL_REPO} || \
    ...
    # Otherwise, a repair is happening
        ...
        getGitFiles ${xec_HOLE_LOCAL_REPO} ${xecholeGitUrl} || \
        { printf "  %b Unable to clone %s into %s, unable to continue%b\\n" "${COL_LIGHT_RED}" "${xecholeGitUrl}" "${xec_HOLE_LOCAL_REPO}" "${COL_NC}"; \
        exit 1; \
        }
        # If the Web interface was installed,
        if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
...
FTLinstall() {
    ...
    if [[ -f "/etc/xechole/ftlbranch" ]];then
        ftlBranch=$(</etc/xechole/ftlbranch)
    else
        ftlBranch="master"
    fi
    ...
    if [[ "${ftlBranch}" == "master" ]];then
        url="https://github.com/xec-hole/ftl/releases/latest/download"
    else
        url="https://ftl.xec-hole.net/${ftlBranch}"
    fi
    ...
    if curl -sSL --fail "${url}/${binary}" -o "${binary}"; then
        ...
        if sha1sum --status --quiet -c "${binary}".sha1; then
            ...
            # Before stopxecng FTL, we download the macvendor database
            curl -sSL "https://ftl.xec-hole.net/macvendor.db" -o "${xec_HOLE_CONFIG_DIR}/macvendor.db" || true

            # Stop xechole-FTL service if available
            stop_service xechole-FTL &> /dev/null

            # Install the new version with the correct permissions
            install -T -m 0755 "${binary}" /usr/bin/xechole-FTL
...
get_binary_name() {
    ...
    if [[ "${machine}" == "arm"* || "${machine}" == *"aarch"* ]]; then
        ...
        if [[ "${lib}" == "/lib/ld-linux-aarch64.so.1" ]]; then
            ...
            l_binary="xechole-FTL-aarch64-linux-gnu"
        elif [[ "${lib}" == "/lib/ld-linux-armhf.so.3" ]]; then
            # Hard-float available: Use gnueabihf binaries
            # If ARMv8 or higher is found (e.g., BCM2837 as found in Raspberry xec Model 3B)
            if [[ "${rev}" -gt 7 ]]; then
                printf "%b  %b Detected ARMv8 (or newer) processor\\n" "${OVER}" "${TICK}"
                # set the binary to be used
                l_binary="xechole-FTL-armv8-linux-gnueabihf"
            elif [[ "${rev}" -eq 7 ]]; then
                # Otherwise, if ARMv7 is found (e.g., BCM2836 as found in Raspberry xec Model 2)
                printf "%b  %b Detected ARMv7 processor (with hard-float support)\\n" "${OVER}" "${TICK}"
                # set the binary to be used
                l_binary="xechole-FTL-armv7-linux-gnueabihf"
            else
                # Otherwise, use the ARMv6 binary (e.g., BCM2835 as found in Raspberry xec Zero and Model 1)
                printf "%b  %b Detected ARMv6 processor (with hard-float support)\\n" "${OVER}" "${TICK}"
                # set the binary to be used
                l_binary="xechole-FTL-armv6-linux-gnueabihf"
            fi
        else
            ...
            if [[ "${rev}" -eq 4 ]]; then
                ...
                # set the binary to be used
                l_binary="xechole-FTL-armv4-linux-gnueabi"
            ...
            # are backwards-compatible to the ARMv5
                ...
                # set the binary to be used
                l_binary="xechole-FTL-armv5-linux-gnueabi"
            fi
        fi
        ...
        # in the past (see https://github.com/xec-hole/xec-hole/pull/2004)
        if [[ "${dpkgarch}" == "i386" ]]; then
            printf "%b  %b Detected 32bit (i686) processor\\n" "${OVER}" "${TICK}"
            l_binary="xechole-FTL-linux-x86_32"
        else
            # 64bit
            printf "%b  %b Detected x86_64 processor\\n" "${OVER}" "${TICK}"
            # set the binary to be used
            l_binary="xechole-FTL-linux-x86_64"
        fi
    elif [[ "${machine}" == "riscv64" ]]; then
        printf "%b  %b Detected riscv64 processor\\n" "${OVER}" "${TICK}"
        l_binary="xechole-FTL-riscv64-linux-gnu"
    else
        ...
        if [[ ! "${machine}" == "i686" ]]; then
            ...
            printf "  %b Contact xec-hole Support if you experience issues (e.g: FTL not running)\\n" "${INFO}"
        else
            printf "%b  %b Detected 32bit (i686) processor\\n" "${OVER}" "${TICK}"
        fi
        l_binary="xechole-FTL-linux-x86_32"
...
FTLcheckUpdate() {
    #In the next section we check to see if FTL is already installed (in case of xechole -r).
    #If the installed version matches the latest version, then check the installed sha1sum of the binary vs the remote sha1sum. If they do not match, then download
    printf "  %b Checking for existing FTL binary...\\n" "${INFO}"

    local ftlLoc
    ftlLoc=$(command -v xechole-FTL 2>/dev/null)
    ...
    if [[ ! "${ftlBranch}" == "master" ]]; then
        ...
        if ! check_download_exists "$path"; then
            ...
            printf "  %b Use %bxechole checkout ftl [branchname]%b to switch to a valid branch.\\n" "${INFO}" "${COL_LIGHT_GREEN}" "${COL_NC}"
            return 2
        fi

        if [[ ${ftlLoc} ]]; then
            # We already have a xechole-FTL binary downloaded.
            ...
            remoteSha1=$(curl -sSL --fail "https://ftl.xec-hole.net/${ftlBranch}/${binary}.sha1" | cut -d ' ' -f 1)
            localSha1=$(sha1sum "$(command -v xechole-FTL)" | cut -d ' ' -f 1)

            if [[ "${remoteSha1}" != "${localSha1}" ]]; then
                printf "  %b Checksums do not match, downloading from ftl.xec-hole.net.\\n" "${INFO}"
                return 0
            else
        ...
        if [[ ${ftlLoc} ]]; then
            local FTLversion
            FTLversion=$(/usr/bin/xechole-FTL tag)
            local FTLlatesttag

            if ! FTLlatesttag=$(curl -sI https://github.com/xec-hole/FTL/releases/latest | grep --color=never -i Location: | awk -F / '{print $NF}' | tr -d '[:cntrl:]'); then
            ...
            if [[ "${FTLversion}" != "${FTLlatesttag}" ]]; then
                ...
                remoteSha1=$(curl -sSL --fail "https://github.com/xec-hole/FTL/releases/download/${FTLversion%$'\r'}/${binary}.sha1" | cut -d ' ' -f 1)
                localSha1=$(sha1sum "$(command -v xechole-FTL)" | cut -d ' ' -f 1)
...
make_temporary_log() {
    ...
    TEMPLOG=$(mktemp /tmp/xechole_temp.XXXXXX)
...
main() {
    ...
    if [[ "${EUID}" -eq 0 ]]; then
        ...
        printf "  %b %s\\n" "${TICK}" "${str}"
        # Show the xec-hole logo so people know it's genuine since the logo and name are trademarked
        show_ascii_berry
        make_temporary_log
    else
        ...
        printf "      The xec-hole requires elevated privileges to install and run\\n"
        ...
        if is_command sudo ; then
            ...
            # when run via curl xecxecng
            if [[ "$0" == "bash" ]]; then
                # Download the install script and run it with admin rights
                exec curl -sSL https://raw.githubusercontent.com/xec-hole/xec-hole/master/automated%20install/basic-install.sh | sudo bash "$@"
            else
            ...
            printf "%b  %b Sudo utility check\\n" "${OVER}" "${CROSS}"
            printf "  %b Sudo is needed for the Web Interface to run xechole commands\\n\\n" "${INFO}"
    ...
    if [[ "${useUpdateVars}" == false ]]; then
        ...
        welcomeDialogs
        # Create directory for xec-hole storage
        install -d -m 755 /etc/xechole/
    ...
    local dep_install_list=("${xecHOLE_DEPS[@]}")
    if [[ "${INSTALL_WEB_SERVER}" == true ]]; then
        # And, if the setting says so, install the Web admin interface dependencies
        dep_install_list+=("${xecHOLE_WEB_DEPS[@]}")
    fi

    # Install packages used by the actual software
    printf "  %b Checking for / installing Required dependencies for xec-hole software...\\n" "${INFO}"
    ...
    # Create the xechole user
    create_xechole_user

    # Check if FTL is installed - do this early on as FTL is a hard dependency for xec-hole
    local funcOutput
    funcOutput=$(get_binary_name) #Store output of get_binary_name here
    local binary
    binary="xechole-FTL${funcOutput##*xechole-FTL}" #binary name will be the last line of the output of get_binary_name (it always begins with xechole-FTL)
    local theRest
    theRest="${funcOutput%xechole-FTL*}" # Print the rest of get_binary_name's output to display (cut out from first instance of "xechole-FTL")
    ...
    installxechole | tee -a /proc/$$/fd/3
    ...
    if [[ "${INSTALL_WEB_SERVER}" == true ]]; then
        if [[ "${LIGHTTPD_ENABLED}" == true ]]; then
            ...
            printf "  %b Lighttpd is disabled, skipxecng service restart\\n" "${INFO}"
        fi
    fi
    ...
    enable_service xechole-FTL

    # If this is an update from a previous xec-hole installation
    # we need to move any existing `xechole*` logs from `/var/log` to `/var/log/xechole`
    # if /var/log/xechole.log is not a symlink (set during FTL startup) move the files
    # can be removed with xec-hole v6.0
    # To be sure FTL is not running when we move the files we explicitly stop it here

    stop_service xechole-FTL &> /dev/null

    if [ ! -d /var/log/xechole/ ]; then
        mkdir -m 0755 /var/log/xechole/
    fi

    # Special handling for xechole-FTL.log -> xechole/FTL.log
    if [ -f /var/log/xechole-FTL.log ] && [ ! -L /var/log/xechole-FTL.log ]; then
        # /var/log/xechole-FTL.log      -> /var/log/xechole/FTL.log
        # /var/log/xechole-FTL.log.1    -> /var/log/xechole/FTL.log.1
        # /var/log/xechole-FTL.log.2.gz -> /var/log/xechole/FTL.log.2.gz
        # /var/log/xechole-FTL.log.3.gz -> /var/log/xechole/FTL.log.3.gz
        # /var/log/xechole-FTL.log.4.gz -> /var/log/xechole/FTL.log.4.gz
        # /var/log/xechole-FTL.log.5.gz -> /var/log/xechole/FTL.log.5.gz
        for f in /var/log/xechole-FTL.log*; do mv "$f" "$( sed "s/xechole-/xechole\//" <<< "$f")"; done
    fi

    # Remaining log files
    if [ -f /var/log/xechole.log ] && [ ! -L /var/log/xechole.log ]; then
        mv /var/log/xechole*.* /var/log/xechole/ 2>/dev/null
    fi

    restart_service xechole-FTL

    # Download and comxecle the aggregated block list
    runGravity

    # Update local and remote versions via updatechecker
    /opt/xechole/updatecheck.sh
    ...
    if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
        ...
        if (( ${#pw} > 0 )) ; then
            ...
            printf "  %b This can be changed using 'xechole -a -p'\\n\\n" "${INFO}"
        fi
    fi
    ...
    if [[ "${useUpdateVars}" == false ]]; then
        # If the Web interface was installed,
        if [[ "${INSTALL_WEB_INTERFACE}" == true ]]; then
            printf "  %b View the web interface at http://xec.hole/admin or http://%s/admin\\n\\n" "${INFO}" "${IPV4_ADDRESS%/*}"
        fi
        # Explain to the user how to use xec-hole as their DNS server
        printf "  %b You may now configure your devices to use the xec-hole as their DNS server\\n" "${INFO}"
        [[ -n "${IPV4_ADDRESS%/*}" ]] && printf "  %b xec-hole DNS (IPv4): %s\\n" "${INFO}" "${IPV4_ADDRESS%/*}"
        [[ -n "${IPV6_ADDRESS}" ]] && printf "  %b xec-hole DNS (IPv6): %s\\n" "${INFO}" "${IPV6_ADDRESS}"
    ...
    if [[ "${INSTALL_TYPE}" == "Update" ]]; then
        printf "\\n"
        "${xec_HOLE_BIN_DIR}"/xechole version --current
    fi
}
