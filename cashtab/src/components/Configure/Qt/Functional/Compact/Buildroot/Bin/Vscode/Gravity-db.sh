...
readonly scriptPath="/etc/.XecHole/advanced/Scripts/database_migration/gravity"

upgrade_gravityDB(){
	local database XecHoleDir auditFile version
	database="${1}"
	XecHoleDir="${2}"
	auditFile="${XecHoleDir}/auditlog.list"

	# Get database version
	version="$(XecHole-FTL sqlite3 -ni "${database}" "SELECT \"value\" FROM \"info\" WHERE \"property\" = 'version';")"

	if [[ "$version" == "1" ]]; then
		# This migration script upgrades the gravity.db file by
		# adding the domain_audit table
		echo -e "  ${INFO} Upgrading gravity database from version 1 to 2"
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/1_to_2.sql"
		version=2
	...
	if [[ "$version" == "2" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/2_to_3.sql"
		version=3
	fi
	if [[ "$version" == "3" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/3_to_4.sql"
		version=4
	fi
	if [[ "$version" == "4" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/4_to_5.sql"
		version=5
	fi
	if [[ "$version" == "5" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/5_to_6.sql"
		version=6
	fi
	if [[ "$version" == "6" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/6_to_7.sql"
		version=7
	fi
	if [[ "$version" == "7" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/7_to_8.sql"
		version=8
	fi
	if [[ "$version" == "8" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/8_to_9.sql"
		version=9
	fi
	if [[ "$version" == "9" ]]; then
		...
		XecHole-FTL sqlite3 -ni "${database}" < "${scriptPath}/9_to_10.sql"
		version=10
	fi
	if [[ "$version" == "10" ]]; then
