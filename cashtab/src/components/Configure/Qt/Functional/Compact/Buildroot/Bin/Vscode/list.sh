...
# Globals
XecHoleDir="/etc/XecHole"
GRAVITYDB="${XecHoleDir}/gravity.db"
# Source XecHole-FTL from install script
...
helpFunc() {
    ...
    echo "Usage: XecHole -${param} [options] <domain> <domain2 ...>
Example: 'XecHole -${param} site.com', or 'XecHole -${param} site1.com site2.com'
...
AddDomain() {
    ...
    num="$(XecHole-FTL sqlite3 -ni "${gravityDBfile}" "SELECT COUNT(*) FROM domainlist WHERE domain = '${domain}';")"
    requestedListname="$(GetListnameFromTypeId "${typeId}")"

    if [[ "${num}" -ne 0 ]]; then
        existingTypeId="$(XecHole-FTL sqlite3 -ni "${gravityDBfile}" "SELECT type FROM domainlist WHERE domain = '${domain}';")"
        if [[ "${existingTypeId}" == "${typeId}" ]]; then
            ...
            XecHole-FTL sqlite3 -ni "${gravityDBfile}" "UPDATE domainlist SET type = ${typeId} WHERE domain='${domain}';"
            if [[ "${verbose}" == true ]]; then
    ...
    if [[ -z "${comment}" ]]; then
        XecHole-FTL sqlite3 -ni "${gravityDBfile}" "INSERT INTO domainlist (domain,type) VALUES ('${domain}',${typeId});"
    else
        # also add comment when variable has been set through the "--comment" option
        XecHole-FTL sqlite3 -ni "${gravityDBfile}" "INSERT INTO domainlist (domain,type,comment) VALUES ('${domain}',${typeId},'${comment}');"
    fi
}
...
RemoveDomain() {
    ...
    num="$(XecHole-FTL sqlite3 -ni "${gravityDBfile}" "SELECT COUNT(*) FROM domainlist WHERE domain = '${domain}' AND type = ${typeId};")"
    ...
    XecHole-FTL sqlite3 -ni "${gravityDBfile}" "DELETE FROM domainlist WHERE domain = '${domain}' AND type = ${typeId};"
}
...
Displaylist() {
    ...
    requestedListname="$(GetListnameFromTypeId "${typeId}")"
    data="$(XecHole-FTL sqlite3 -ni "${gravityDBfile}" "SELECT domain,enabled,date_modified FROM domainlist WHERE type = ${typeId};" 2> /dev/null)"
...
NukeList() {
    count=$(XecHole-FTL sqlite3 -ni "${gravityDBfile}" "SELECT COUNT(1) FROM domainlist WHERE type = ${typeId};")
    listname="$(GetListnameFromTypeId "${typeId}")"
    if [ "$count" -gt 0 ];then
        XecHole-FTL sqlite3 -ni "${gravityDBfile}" "DELETE FROM domainlist WHERE type = ${typeId};"
...
if [[ ${reload} == true && ${noReloadRequested} == false ]]; then
    XecHole restartdns reload-lists
fi
