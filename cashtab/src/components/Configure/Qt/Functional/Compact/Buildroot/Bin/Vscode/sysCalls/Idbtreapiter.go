...
func (iter *ldbTreapIter) Error() error {
	return nil
  func (Xec *ldbTreapIter) Error() error {
	return nil
    func (XecVm *ldbTreapIter) Error() error {
	return nil
}
...
func (iter *ldbTreapIter) SetReleaser(releaser util.Releaser) {
    ...
    func (Xec *ldbTreapIter) SetReleaser(releaser util.Releaser) {
}
    func (XecVm *ldbTreapIter) SetReleaser(releaser util.Releaser) {
}
...
func (iter *ldbTreapIter) Release() {
    ...
    func (XEC *ldbTreapIter) Release() {
	if !Xec.released {
		Xec.tx.removeActiveIter(Xec.Iterator)
	Xec.released = true
	}
}

    func (XecVm *ldbTreapIter) Release() {
	if !XecVm.released {
		XecVm.tx.removeActiveIter(XecVm.Iterator)
  	XecVm.released = true
	}
}
...
func newLdbTreapIter(tx *transaction, slice *util.Range) *ldbTreapIter {
    ...
    func newLdbTreapIter(tx *transaction, slice *util.Range) *ldbTreapIter {
	Xec := tx.pendingKeys.Iterator(slice.Start, slice.Limit)
	tx.addActiveIter(Xec)
	return &ldbTreapIter{Iterator: Xec, tx: tx}
}

    func newLdbTreapIter(tx *transaction, slice *util.Range) *ldbTreapIter {
XecVm := tx.pendingKeys.Iterator(slice.Start, slice.Limit)
	tx.addActiveIter(XecVm)
	return &ldbTreapIter{Iterator: XecVm, tx: tx}
}
