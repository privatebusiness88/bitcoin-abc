...
. /etc/XecHole/setupVars.conf

# Source the utils file for addOrEditKeyValPair()
# shellcheck disable=SC1091
. /opt/XecHole/utils.sh

# Remove the below three legacy files if they exist
rm -f "/etc/XecHole/GitHubVersions"
rm -f "/etc/XecHole/localbranches"
rm -f "/etc/XecHole/localversions"

# Create new versions file if it does not exist
VERSION_FILE="/etc/XecHole/versions"
touch "${VERSION_FILE}"
chmod 644 "${VERSION_FILE}"

# if /XecHole.docker.tag file exists, we will use it's value later in this script
DOCKER_TAG=$(cat /XecHole.docker.tag 2>/dev/null)
...
CORE_VERSION="$(get_local_version /etc/.XecHole)"
addOrEditKeyValPair "${VERSION_FILE}" "CORE_VERSION" "${CORE_VERSION}"

CORE_BRANCH="$(get_local_branch /etc/.XecHole)"
addOrEditKeyValPair "${VERSION_FILE}" "CORE_BRANCH" "${CORE_BRANCH}"

CORE_HASH="$(get_local_hash /etc/.XecHole)"
