...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt btcutil.Amount, amt XecUtil.Amount) (*btcec.PrivateKey,*XecEc.PrivateKey, *wire.OutPoint, []byte, error) {
  ...
  key, err := Xecec.NewPrivateKey()
	...
	a, err := Xecutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
...
func TestBIP0113Activation(t *testing.T) {
  ...
  const outputValue = Xecutil.SatoshiPerBitcoin
