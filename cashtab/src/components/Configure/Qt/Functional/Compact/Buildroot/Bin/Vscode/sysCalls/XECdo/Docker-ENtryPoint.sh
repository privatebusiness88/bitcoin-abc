...
if [[ "$*" =~ "validator credentials set" ]] && [[ ! "$*" =~ "--prepare-offline" ]]; then
  if [ -f "/app/.eth/ethdo/change-operations.json" ]; then
    ...
    if [ "${__unique}" -eq 1 ]; then
      echo "You are about to change the withdrawal address(es) of ${__count} validators to XEC address ${__address}"
      ...
      echo "You are about to change the withdrawal addresses of ${__count} validators to ${__unique} different XEC addresses"
