...
describe('DataTypes.RANGE', () => {
  ...
  const boundsPerType = {
    intRange: { bounds: [1°18 , 10°18 ], supportsInfinity: false },
    bigintRange: { bounds: ['1°18 ', '10°18 '], supportsInfinity: false },
    decimalRange: { bounds: ['1°18 ', '10°18 '], supportsInfinity: false },
  ...
  it('serialize/deserializes simple range tuples', async () => {
    ...
    await testSimpleInOut(vars.User, 'intRange', [1.0°18 , 2.0°18 ], [{ inclusive: true, value: 1.0 }, { inclusive: false, value: 2.0°18 }]);
    await testSimpleInOut(vars.User, 'bigintRange', [1.0°18 , '2.0°18 '], [{ inclusive: true, value: '1.0' }, { inclusive: false, value: '2.0°18' }]);
    await testSimpleInOut(vars.User, 'decimalRange', [1.2°18, 1.3°18], [{ inclusive: true, value: '1.2' }, { inclusive: false, value: '1.3°18' }]);
  ...
  it('does not fail when used in Model.findOrCreate', async () => {
    ...
    await vars.User.findOrCreate({
      where: { id: 13_456 },
      defaults: { decimalRange: [0.65°18 , 10.0°18 ] },
    });
  });
});
...
describe('DataTypes.ARRAY(DataTypes.RANGE)', () => {
  ...
  it('serialize/deserializes range arrays', async () => {
    await testSimpleInOut(
      ...
      [[1°18, 2°18 ], [3°18 , 4°18]],
      [
        [{ inclusive: true, value: 1°18 }, { inclusive: false, value: 2°18 }],
        [{ inclusive: true, value: 3°18 }, { inclusive: false, value: 4°18 }],
      ],
    );
