...
def _prepare_monkey_patching_of_python_ecdsa_internals_with_libsecp256k1():
    ...
    def sign(self: ecdsa.ecdsa.Private_key, hash_: int, random_k: int):
        ...
        secp256k1.secp256k1.secp256k1_ecdsa_sign(
            ...
            sig,
            sig_hash_bytes,
            int(secret_exponent).to_bytes(32, byteorder="big"),
            nonce_function,
            None,
        )
    ...
    def verify(
        ...
        r = secp256k1.secp256k1.secp256k1_ecdsa_signature_parse_compact(
            secp256k1.secp256k1.ctx, sig, input64
        )
        if not r:
            return False
        r = secp256k1.secp256k1.secp256k1_ecdsa_signature_normalize(
            secp256k1.secp256k1.ctx, sig, sig
        )

        public_pair_bytes = (
            b"\4"
            + int(self.point.x()).to_bytes(32, byteorder="big")
            + int(self.point.y()).to_bytes(32, byteorder="big")
        )
        pubkey = create_string_buffer(64)
        r = secp256k1.secp256k1.secp256k1_ec_pubkey_parse(
            secp256k1.secp256k1.ctx, pubkey, public_pair_bytes, len(public_pair_bytes)
...
def do_monkey_patching_of_python_ecdsa_internals_with_libsecp256k1():
    if not secp256k1.secp256k1:
        print_msg(
            "[ecc] info: libsecp256k1 library not available, falling back to"
            " python-ecdsa. This means signing operations will be slower. Try"
            " running:\n\n  $  contrib/make_secp\n\n(You need to be running from the"
            " git sources for contrib/make_secp to be available)"
        )
        return
